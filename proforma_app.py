...
if LANG == "en":
    ...
    # Heading text as it appears in the English template — must be bolded
    ...
    L = dict(
        date="Date", client="2. Client", pick="Pick existing customer or fill in manually",
        ...
        gen="📥 Generate Proforma Invoice",
        wcomp="Please enter a company name.",
        witems="Please add at least one line item.",
        # Friendly attn validation — English
        wattn="Please enter the contact's full name. It is required when 'To the attention of' is selected.",
    ...
    TOTAL_TPL = "TOTALE \u2013 {dt} \u2013"
    # Heading text as it appears in the Italian template — must be bolded
    ...
    L = dict(
        date="Data", client="2. Cliente", pick="Seleziona cliente o compila manualmente",
        ...
        gen="📥 Genera Fattura Proforma",
        wcomp="Inserire la ragione sociale.",
        witems="Aggiungere almeno un articolo.",
        # Friendly attn validation — Italian
        wattn="Inserisci Nome contatto. È obbligatorio se selezioni 'All'attenzione di'.",
...
# FIX: use isoformat() to avoid timezone/DST off-by-one when saving to Supabase.
# The date_input widget returns a datetime.date — format it directly, no conversion.
...
# FIX: file name format "PI012-26" — replace "/" with "-", no "proforma " prefix
default_doc_name = pnum.replace("/", "-")
if company.strip():
    default_doc_name += f" {company.strip()}"
doc_name = st.text_input(L["fname"], value=default_doc_name)
...
if st.button(L["gen"], type="primary", use_container_width=True, disabled=not number_ok):
    ...
    for para in doc.paragraphs:
        ...
        if "PROFORMA INVOICE NO" in full or "FATTURA PROFORMA N" in full or \
           ("PROFORMA" in full.upper() and "N" in full.upper() and pnum in full):
            for r in para.runs:
                r.bold=True; r.font.name="Verdana"; r.font.size=Pt(10)
            continue

        if para.text.strip().startswith("To the attn.") or "All'attenzione" in full:
            if include_attn and full_name.strip():
                para.clear()
                attn_text = f"To the attn. of {sal} {full_name}".strip().replace("  ", " ")
